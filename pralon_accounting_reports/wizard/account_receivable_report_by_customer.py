...
class wizard(wizard):
    ...
    def get_receivable(self, cr, uid, form, context=None):
        ...
        sql_receive = ('SELECT aml.partner_id,'
                        ' COALESCE(sum(COALESCE(aml.debit, 0.0) - COALESCE(aml.credit, 0.0)), 0.0)'
                        ' FROM account_move_line aml'
                        " JOIN account_account aa ON (aa.id = aml.account_id AND aa.type IN ('receivable'))"
                        ' WHERE aml.partner_id IN (%s) '
                        ' GROUP BY aml.partner_id, aml.account_id')
        cr.execute(sql_receive % (','.join(str(cust_id)
    ...
    def get_payable(self, cr, uid, form, context=None):
        ...
        sql_receive = ('SELECT aml.partner_id, '
                        ...
                        ' FROM account_move_line aml'
                        ' WHERE aml.partner_id IN (%s)'
                        ' AND aml.account_id IN (SELECT id '
                        ' FROM account_account WHERE'
                        " type IN ('payable')"
                        ' UNION'
                        ' SELECT rc.payment_holding_account'
                        ' FROM res_company rc'
                        ' WHERE rc.id = %s)'
    ...
    def get_currency_company(self, cr, uid, form, context=None):
        sql_curr_comp =  ('SELECT rcu.symbol'
                            ' FROM res_company rc'
                            ' JOIN res_currency rcu ON rcu.id = rc.currency_id'
                            ' WHERE rc.id = %s')
        cr.execute(sql_curr_comp % (','.join(str(com_id.id)
                                           for com_id in form.company_ids),))
        rv = cr.fetchall()
        if len(rv) !=0:
            for item in rv:
