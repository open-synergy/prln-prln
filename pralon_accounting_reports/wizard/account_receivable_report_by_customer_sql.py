...
WITH receivable_accounts AS
    ...
    (SELECT ai.id AS inv_id,
            ...
            ai.address_invoice_id AS address_id,
            ai.date_invoice AS date_invoice,
            ai.partner_id AS partner,
            (CASE WHEN ai.currency_id != rc.currency_id
              THEN CONCAT ('(',rcy.symbol, ' ', ai.amount_untaxed,')')
              ELSE NULL END) AS note_untaxed,
            (CASE WHEN ai.currency_id != rc.currency_id
              THEN CONCAT ('(',rcy.symbol, ' ', ai.amount_tax,')')
              ELSE NULL END) AS note_tax,
            (CASE WHEN ai.currency_id != rc.currency_id
              THEN CONCAT ('(',rcy.symbol, ' ', ai.amount_total,')')
              ELSE NULL END) AS note_total,
            (CASE WHEN ai.currency_id != rc.currency_id
              ...
              ELSE ai.amount_untaxed END) AS amount_untaxed,
            (CASE WHEN ai.currency_id != rc.currency_id
              THEN ai.amount_tax /
               (SELECT rate
                FROM res_currency_rate
                WHERE currency_id = ai.currency_id
                  AND name <= ai.date_invoice
                ORDER BY name DESC LIMIT 1)
              ELSE ai.amount_tax END) AS amount_tax,
            (CASE WHEN ai.currency_id != rc.currency_id
              ...
              ELSE ai.residual END) AS inv_residual,
            (CASE WHEN ai.currency_id != rc.currency_id
              THEN CONCAT('(',rcy.symbol,' ', ai.residual, ')')
              ELSE NULL END) AS note_inv_residual,
            (CASE WHEN ai.type = 'out_invoice'
              THEN SUM(aml.debit)
              ELSE SUM(aml.debit) * -1 END) AS amount_total,
            ai.state AS state,
     ...
     FROM
       (SELECT ai1.id,
        ...
        JOIN account_move_line aml1 ON ai1.move_id = aml1.move_id
        AND aml1.debit !=0 AND aml1.debit IS NOT NULL
        AND aml1.account_id IN
            (SELECT id
        ...
        JOIN account_move_line aml2 ON ai2.move_id = aml2.move_id
        AND aml2.credit !=0 AND aml2.credit IS NOT NULL
        AND aml2.account_id IN
            (SELECT id
               FROM receivable_accounts)
        JOIN account_move am2 ON am2.id = ai2.move_id
        JOIN account_journal acj2_1 ON acj2_1.id = am2.journal_id
        AND acj2_1.type = 'sale_refund'
        JOIN account_move_line aml_rc ON aml2.id != aml_rc.id
        AND COALESCE(aml_rc.reconcile_id, aml_rc.reconcile_partial_id) = COALESCE(aml2.reconcile_id, aml2.reconcile_partial_id)
        AND aml2.account_id IN
            (SELECT id
               FROM receivable_accounts)
        JOIN account_journal acj2_2 ON acj2_2.id = aml_rc.journal_id
        ...
        JOIN account_move_line aml3 ON ai3.move_id = aml3.move_id
        AND aml3.credit !=0 AND aml3.credit IS NOT NULL
        AND aml3.reconcile_id IS NULL AND aml3.reconcile_partial_id IS NULL
        AND aml3.account_id IN
           (SELECT id
              FROM receivable_accounts)
        JOIN account_move am3 ON am3.id = ai3.move_id
        JOIN account_journal acj3_1 ON acj3_1.id = am3.journal_id
     ...
     LEFT JOIN res_partner_address rpa ON rpa.id = ai.address_invoice_id
     LEFT JOIN res_partner rp ON rp.id = rpa.partner_id
     LEFT JOIN res_partner rp2 ON rp2.id = ai.partner_id
     LEFT JOIN res_country_state rcs ON rpa.state_id = rcs.id
    ...
    (SELECT ai.id AS inv_id,
     ...
     LEFT JOIN account_move_line br ON ai.move_id = br.move_id
     AND br.account_id IN
         (SELECT id
            FROM receivable_accounts)
     LEFT JOIN account_move_line payment ON payment.id != br.id
     AND COALESCE(br.reconcile_id, br.reconcile_partial_id) = COALESCE(payment.reconcile_id, payment.reconcile_partial_id)
     AND payment.account_id IN
         (SELECT id
            FROM receivable_accounts)
  ...
  WHERE id.state IN ('open',
                     'paid')
    AND id.date_invoice::DATE BETWEEN '$P!{FROM_DATE_2_YR}-$P!{FROM_DATE_2_MO}-$P!{FROM_DATE_2_DY}'
    AND '$P!{TO_DATE_2_YR}-$P!{TO_DATE_2_MO}-$P!{TO_DATE_2_DY}'
    AND id.inv_type IN ('out_invoice', 'out_refund')
    AND id.company IN ($P!{COMPANY_IDS})
        $P!{CUSTOMER_ADDR_IDS}
